...
bool RgbdSensor_nws_ros::initialize_ROS(yarp::os::Searchable &params)
{
    ...
    rosNode = new yarp::os::Node(node_name);
    nodeSeq = 0;

    std::string rgb_data_topic_name = color_topic_base_name + "/image_rect_color";
    if (!rosPublisherPort_color.topic(rgb_data_topic_name))
    {
    ...
    if (!rosPublisherPort_depth.topic(depth_data_topic_name))
    {
    ...
    if (!rosPublisherPort_colorCaminfo.topic(rgb_info_topic_name))
    {
    ...
    if (!rosPublisherPort_depthCaminfo.topic(depth_info_topic_name))
    {
